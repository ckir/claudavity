...
async def create_worktree(target_dir: str, task_id: str) -> str:
    ...
    if code != 0:
        raise ValueError(f"Target directory {target_dir} is not a valid git repository. Error: {err}")
    ...
    code, _, err = await run_git_command(target_dir, 'worktree', 'add', worktree_path, '-b', branch_name)
    if code != 0:
...
async def cleanup_worktree(target_dir: str, task_id: str, success: bool = False):
    ...
    if success:
        ...
        if code != 0:
            ...
            raise RuntimeError(f"Merge conflict detected. The branch {branch_name} has been preserved for manual review. Git error: {err}")
